...
import { dbFilePath } from '../constant/index';
...
const createTable = (versionResult) => {
	db.serialize(() => {
		...
		// 专注记录表
		db.run(`CREATE TABLE IF NOT EXISTS focus_plan_log  (
            id                          INTEGER PRIMARY KEY AUTOINCREMENT,
            create_time                 TIMESTAMP default (datetime('now', 'localtime')),
            title                       VARCHAR (50) NOT NULL,
            progress_time               VARCHAR (50) NOT NULL,
            total_time                  VARCHAR (50) NOT NULL
        );`);

		//工作台菜单表
		db.run(`CREATE TABLE IF NOT EXISTS work_menu (
            id        INTEGER      PRIMARY KEY AUTOINCREMENT,
		...
		//工作台链接表
		db.run(`CREATE TABLE IF NOT EXISTS work_link (
            id          INTEGER      PRIMARY KEY AUTOINCREMENT,
            menu_key    VARCHAR (50) NOT NULL, 
